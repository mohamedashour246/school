                        ...
                        <div class="row">
                            <div class="col-md-6 border-right-2 border-right-blue-400">
                                ...
                                <div class="form-group row">
                                    ...
                                    <div class="col-lg-9">
                                        <select data-placeholder="Choose..." required name="session" id="current_session" class="select-search form-control">
                                            <option value=""></option>
                                            @for($y=date('Y', strtotime('- 3 years')); $y<=date('Y', strtotime('+ 1 years')); $y++)
                                                <option {{ ($setting['session'] == (($y-=1).'-'.($y+=1))) ? 'selected' : '' }}>{{ ($y-=1).'-'.($y+=1) }}</option>
                                            @endfor
                                        </select>
                                    </div>
                                </div>
                                ...
                                <div class="form-group row">
                                    ...
                                    <div class="col-lg-9">
                                        <input name="end_first_term" value="{{ $setting['first_term'] }}" type="text" class="form-control date-pick" placeholder="Date Term Ends">
                                    </div>
                                </div>
                                <div class="form-group row">
                                    <label class="col-lg-2 col-form-label font-weight-semibold">نهاية الترم الثاني</label>
                                    <div class="col-lg-9">
                                        <input name="end_second_term" value="{{ $setting['second_term'] }}" type="text" class="form-control date-pick" placeholder="Date Term Ends">
                                    </div>
                                </div>
                                ...
                                <div class="form-group row">
                                    ...
                                    <div class="col-lg-9">
                                        <div class="mb-3">
                                            <img id="output" style="width: 100px" height="100px" src="{{ URL::asset('attachments/logo/'.$setting['logo']) }}" alt="">
                                        </div>
                                        <input name="logo" accept="image/*" type="file" class="file-input" onchange="loadfile(event)" data-show-caption="false" data-show-upload="false" data-fouc>
                                    </div>
                                </div>
                            </div>
                        </div>
